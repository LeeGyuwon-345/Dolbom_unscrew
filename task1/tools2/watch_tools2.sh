...
TOOLS=/home/leegyuwon/Documents/task1/tools2
...
trainer_pid() {
  for p in $(pgrep -f "main/rl/train.py" 2>/dev/null); do
    ...
    case "$c" in
      /home/leegyuwon/*python*experiment=CapUnscrew2DG5FS_${TAG}_*) echo "$p"; return;;
    esac
  done
}

latest_log() { ls -t "${LOGS}/cap_unscrew2_${TAG}_"*.log 2>/dev/null | head -1; }
latest_ckpt() { ls -t "${RUNS}/CapUnscrew2DG5FS_${TAG}_"*/nn/*.pth 2>/dev/null | head -1; }
...
epochs_done() {
  local best=0 n
  for l in "${LOGS}/cap_unscrew2_${TAG}_"*.log; do
    [[ -f "$l" ]] || continue
...
while :; do
  ...
  if [[ -n "$ck" ]]; then
    RESAMPLE_WRIST=1 MAX_ITER="$remain" NUM_ENVS=512 DIAG_EVERY=200 TAG="$TAG" CKPT="$ck" \
      setsid nohup ./train_cap_unscrew2.sh > /dev/null 2>&1 < /dev/null &
  else
