...
asset = gym.load_asset(sim, HERE, "rb5_allegro.urdf", ao)
...
tf = gymapi.Transform(); tf.p = gymapi.Vec3(0.8, 0.0, 0.0)   # 텀블러 바닥 = base 와 같은 z, x 800mm
gym.create_actor(env, tum, tf, "tumbler", 0, 2)
...
tgt = torch.zeros(nd)
if "joint_12.0" in names:
    tgt[names.index("joint_12.0")] = 0.263   # 엄지 CMC 하한 (0 이 범위 밖)
sweep = False
t = 0.0
