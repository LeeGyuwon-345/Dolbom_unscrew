...
def solve_from(q0, p_t, R_t, iters=120, lr=0.06):
    ...
    for _ in range(iters):
        ...
        # 손목 감김 417도 > wrist3 예산이라 완전 자세 추종은 원리적 불가.
        # 공구축 방향만 정렬하고 축 주위 스핀은 자유로 둔다.
        ax = m[:3, :3] @ torch.tensor([0., -1., 0.])   # tcp -y = 플랜지 바깥
        ax_t = R_t @ torch.tensor([0., -1., 0.])
        loss = ((m[:3, 3] - p_t) ** 2).sum() * 4.0 + ((ax - ax_t) ** 2).sum() * 1.0 \
    ...
    with torch.no_grad():
        ...
        ax = m[:3, :3] @ torch.tensor([0., -1., 0.])
        ax_t = R_t @ torch.tensor([0., -1., 0.])
        eo = float(torch.arccos((ax @ ax_t).clamp(-1, 1)))
        minz = min(float(ms[l].get_matrix()[0, 2, 3]) for l in ("link3", "link4", "link5"))
        margin = float(torch.minimum(qc - lo, hi - qc)[:5].min())  # wrist3 제외
