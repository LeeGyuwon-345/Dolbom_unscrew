...
URDF = _PKG + "/assets/dg5fs_hand/dg5fs_right.urdf"
MESH_DIR = _PKG + "/assets/dg5fs_hand/meshes/dg5fs_right"
...
class HandFK:
    ...
    def build_capsules(self, n_samples: int = 400):
        ...
        for name in self.all_links:
            # 뿌리(link_X_1)는 뺀다. 손바닥에 나란히 장착된 넓적한 블록이라
            # 캡슐 반경이 19~31mm 로 나오고, 설계상 원래 이웃해 있어서 넣으면
            # 정상 자세도 -33mm "겹침" 으로 읽힌다. 서로 부딪힐 수 있는 것은
            # 움직이는 마디(_2~tip)다.
            if name == "link_base" or name.endswith("_1"):
                continue
            ...
            # 최대값이 아니라 90퍼센타일. 최대값은 사각 단면의 모서리까지
            # 포함해 반폭 7mm 마디를 11.7mm 로 부풀렸고, 그 과장이 양쪽에서
            # 더해져 실제 0.1mm 겹침을 15mm 로 읽었다. 나머지 10% 모서리는
            # MIN_GAP_LINK 4mm 여유가 흡수한다.
            r = float(np.percentile(
                np.linalg.norm(pts - t[:, None] * p1[None], axis=1), 90.0))
