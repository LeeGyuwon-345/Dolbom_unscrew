...
class GraspInitializer:
    ...
    def __init__(self, device, n_hand_dofs: int):
        ...
        # 손목 노이즈. 위치는 m, 자세는 rad (무작위 축 소회전). 기본 0 = 기존 동작.
        self.wrist_noise_pos = float(os.environ.get("CAP_GRASP_INIT_WRIST_NOISE_POS", "0.0"))
        self.wrist_noise_rot = float(os.environ.get("CAP_GRASP_INIT_WRIST_NOISE_ROT", "0.0"))
        # 사전 계산 노이즈 풀 (make_init_pool.py). 지정되면 위의 런타임 노이즈
        # 대신 풀에서 무작위 인덱싱한다 -- 침투/자기충돌 합격 샘플만 들어 있다.
        self.pool = None
        pool_path = os.environ.get("CAP_GRASP_INIT_POOL", "").strip()
        if pool_path:
            d = torch.load(pool_path, map_location=device)
            self.pool = {
                "dof_pos": d["dof_pos"].to(device=device, dtype=torch.float32),
                "wrist_rel_pos": d["wrist_rel_pos"].to(device=device, dtype=torch.float32),
                "wrist_quat": d["wrist_quat"].to(device=device, dtype=torch.float32),
            }
            print(
                f"[grasp-init] 노이즈 풀 {os.path.basename(pool_path)} "
                f"{self.pool['dof_pos'].shape[0]}개  ({d.get('note', '')})",
                flush=True,
            )
        ...
        if path:
            ...
            print(
                ...
                f"노이즈 {math.degrees(self.noise):.1f}도  "
                f"손목노이즈 {self.wrist_noise_pos*1000:.1f}mm/"
                f"{math.degrees(self.wrist_noise_rot):.1f}도  ({self.pose.note})",
                flush=True,
            )

    @property
    def enabled(self) -> bool:
        return (self.pose is not None or self.pool is not None) and self.frac > 0.0
    ...
    def apply(self, ids: torch.Tensor, dof_pos: torch.Tensor, base_state: torch.Tensor,
              cap_pos: torch.Tensor, write_base: bool = True) -> None:
        ...
        if self.pool is not None:
            idx = torch.randint(
                self.pool["dof_pos"].shape[0], (ids.numel(),), device=ids.device
            )
            dof_pos[ids] = self.pool["dof_pos"][idx]
            if write_base:
                base_state[ids, :3] = cap_pos[ids] + self.pool["wrist_rel_pos"][idx]
                base_state[ids, 3:7] = self.pool["wrist_quat"][idx]
        ...
        dof_pos[ids] = q
        if not write_base:
            # 팔 모드: 베이스는 고정이고 손목은 팔 관절이 만든다.
            return
        wp = cap_pos[ids] + p.wrist_rel_pos.unsqueeze(0)
        if self.wrist_noise_pos > 0:
            wp = wp + torch.randn_like(wp) * self.wrist_noise_pos
        wq = p.wrist_quat.unsqueeze(0).expand(ids.numel(), -1)
        if self.wrist_noise_rot > 0:
            aa = torch.randn(ids.numel(), 3, device=wq.device) * self.wrist_noise_rot
            ang = aa.norm(dim=-1, keepdim=True).clamp_min(1e-9)
            axis = aa / ang
            half = 0.5 * ang
            dq = torch.cat([axis * torch.sin(half), torch.cos(half)], dim=-1)  # xyzw
            wq = _quat_mul_xyzw(dq, wq)
        base_state[ids, :3] = wp
        base_state[ids, 3:7] = wq
        base_state[ids, 7:13] = 0.0


def _quat_mul_xyzw(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """월드 기준 소회전 a 를 자세 b 에 합성한다 (a ⊗ b, xyzw)."""
