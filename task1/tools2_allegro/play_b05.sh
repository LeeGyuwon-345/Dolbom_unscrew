#!/bin/bash
# B05 PLAY (B04 웜스타트 + GRIP_FLOOR 2 + SLIP2_W 7.0). 최신 ckpt 자동.
# 지그 노이즈 기본 off. 켜려면 NOISE=2 bash play_b05.sh
CKPT=${CKPT:-$(ls -t /home/leegyuwon/Documents/ManipTrans/runs/CapUnscrewArm_B05_2026*/nn/last_*ep_*.pth | head -1)}
echo "[play] $(basename $CKPT)  noise=${NOISE:-0}mm"
exec env PLAY=1 CKPT=$CKPT CAP_TUMBLER_POS_NOISE_MM=${NOISE:-0} CAP_SLIP2_BACK_DEAD_MM=2 CAP_SLIP2_BACK_SR_RAMP=0.5 CAP_FRIC_AUDIT=1 TUMBLER_POS=0.4,0,0.022 CAP_Z=0.237 CAP_RESIST_AUDIT=1 CAP_PRIV_RESIST=1 HIDE_RISE=0 CAP_REL_HOME_XY=1 CAP_OBS_COMPACT=1 CAP_ACTOR_PRIV_KEEP=22 PALM_DOWN_SR_RAMP=0.5 PALM_DOWN_W=3.0 PALM_DOWN_MAX=15 OPPOSE_W=2.0 OPPOSE_SEP=140 CAP_HIDE_CAP_QUAT=1 CAP_HIDE_CAP_VEL=1 CAP_REL_SPIN_FREE=1 CAP_BAND_LO=0.012 CAP_BAND_LO_SLACK_MM=2 CAP_RADIUS_LO=0.044 CAP_RADIUS_HI=0.044 CAP_CSCALE_FLOOR=0.25 CAP_GRIP_FLOOR=2,2,2,2 CAP_GRIP_FLOOR_RAMP=4.0 GRIP_W=2.0 CAP_GRIP_MEAN4=1 CONTACT_REF=5 GRASP_STABLE_MIN=0.10 CAP_SLIP2_W=7.0 CAP_SLIP2_DEAD_MM=8 CAP_SLIP2_REF_MM=45 CAP_SLIP2_CONTACT_N=0.3 EPISODE_STEPS=360 CAP_GRAVITY_RAMP_STEPS=1920 CAP_ENERGY_W_HAND=1.0 CAP_ENERGY_W_ARM=1.667 CAP_ENERGY_ALPHA=0.1 NUM_ENVS=1 MAX_ITER=20000 TAG=B05PLAY PYTHONUNBUFFERED=1 HEADLESS=false \
