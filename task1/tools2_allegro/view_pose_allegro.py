...
if argv:
    ...
    cands = sorted(glob.glob(os.path.join("/home/leegyuwon/Documents/task1/tools2_allegro/poses", "*.json")), key=os.path.getmtime)
    if not cands:
...
hand = gym.load_asset(sim, "/home/leegyuwon/Documents/ManipTrans/maniptrans_envs/assets/allegro_hand", "allegro_hand_right.urdf", ho)
tum = None
if WITH_TUM:
    ...
    tum = gym.load_asset(sim, "/home/leegyuwon/Documents/task1/assets/tumbler",
                         "tumbler.urdf", to)
...
# allegro mesh 는 재질색이 없어 검게 나온다 -- 밝은 회백색으로.
for _b in range(gym.get_actor_rigid_body_count(env, act)):
    gym.set_rigid_body_color(env, act, _b, gymapi.MESH_VISUAL, gymapi.Vec3(0.92, 0.92, 0.92))
...
for b, n in enumerate(bn0):
    if True:  # allegro: 보기 전용이라 자기충돌 전부 끔
...
tip_i = [off + bn.index(n) for n in ("link_3.0_tip", "link_7.0_tip", "link_11.0_tip", "link_15.0_tip")]
...
def report():
    ...
    a = [math.degrees(math.atan2(float(rb[tip_i[k], 1]), float(rb[tip_i[k], 0]))) for k in (3, 1)]
    s = abs(a[0] - a[1])
