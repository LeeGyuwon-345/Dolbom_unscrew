#!/usr/bin/env bash
# A39 재생 전용 (720스텝, 리미터 없음) 래퍼 — 학습 프로세스(/proc environ 실측)와 동일 env 보장.
#
...
#
# 사용법:
#   ./play_a39.sh <CKPT.pth>                # GUI (기본 1 env)
#   HEADLESS=true NUM_ENVS=64 ./play_a39.sh <CKPT.pth>   # 헤드리스 프로브
set -euo pipefail
CKPT="${1:?사용법: ./play_a39.sh <checkpoint.pth>}"
