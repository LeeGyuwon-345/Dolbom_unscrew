...
TOOLS_DIR=/home/leegyuwon/Documents/task1/tools2_allegro
...
exp="CapUnscrewArm_${TAG}_${stamp}"
train_log="${LOG_DIR}/cap_unscrew_arm_${TAG}_${stamp}.log"
...
# hand-obs 정합 모드: 네트워크 인코더 input_dim 이 hydra resolver 로 dexhand
...
if [[ "${ARM_HAND_OBS:-0}" == "1" ]]; then
  handobs_args+=(
    ...
    rl_train.params.network.dict_feature_encoder.extractors.proprioception.input_dim=61
  )
fi
redirect=1
if [[ "${PLAY}" == "1" ]]; then
  [[ -n "${CKPT:-}" ]] || { echo "PLAY=1 requires CKPT=<.pth>" >&2; exit 2; }
  exp="CapUnscrewArm_${TAG}_PLAY_${stamp}"
...
exec env \
  ...
  DEXHAND_SELF_COLLISION_MASK="${SELF_COLLISION_MASK:-}" \
  `# 자기충돌 켬 (필터 0). rest/파지 유령 접촉 0 실측 (2026-08-06) -- 마스크 불필요` \
  DEXHAND_COLLISION_FILTER="${COLLISION_FILTER:-0}" \
  CAP_GRASP_TUMBLER=1 \
  ...
  `# 사용자 제공 grasps.csv r50/h17 파지 (FK 검증: 엄지 +0.6mm cos0.95, 3/4 밴드 안)` \
  CAP_GRASP_INIT_POSE="${INIT_POSE:-/home/leegyuwon/Documents/task1/tools2_allegro/poses/arm_grasp_init.json}" \
  ...
  CAP_PALM_LOCAL_AXIS="${PALM_LOCAL_AXIS:-1,0,0}" \
  `# allegro 4지: env 5번째 슬롯 = 약지 중복 -> 보상에서 이중 계산 차단` \
  CAP_DUP_LAST_FINGER=1 \
  `# --- 팔 통합 모드 ---` \
  CAP_ARM_MODE=1 \
  `# 팔 관절 델타 상한 0.05 rad/step = 172도/s (실기 스펙 180도/s 안).` \
  `# 손목 상한(5mm/0.05rad/step) 자코비안 역산: yaw 0.047, pitch 0.051 필요` \
  `# -- 0.05 로 통일 (사용자 확정 2026-08-07). 관절별 리스트도 지원.` \
  CAP_ACTION_DELTA_SCALE_ARM="${ACTION_DELTA_SCALE_ARM:-0.05}" \
  `# 팔 드라이브: 정적 처짐 8.9mm 실측 게인 + 추정 effort (BASELINE 참조)` \
  CAP_ARM_STIFFNESS="${ARM_STIFFNESS:-4000}" \
  CAP_ARM_DAMPING="${ARM_DAMPING:-160}" \
  CAP_ARM_EFFORT="${ARM_EFFORT:-150,150,150,28,28,28}" \
  `# IK 래퍼: 정책 앞 6차원 = 손목 작업공간 델타(hand-only 와 같은 기하),` \
  `# 자코비안 DLS 가 팔 관절 델타로 환산. 관절공간 직접 학습 정체의 처방.` \
  CAP_ARM_IK_WRAPPER="${ARM_IK_WRAPPER:-0}" \
  CAP_ARM_IK_DPOS="${ARM_IK_DPOS:-0.005}" \
  CAP_ARM_IK_DROT="${ARM_IK_DROT:-0.05}" \
  `# DLS 감쇠. 0.05 는 방향 편향(+z 명령에 -x 누설 11도), 0.01 은 요동.` \
  `# 0.02 + 방향보존 스케일링에서 yaw 오차 5%/축오차 1도 실측 (IKDir9).` \
  CAP_ARM_IK_LAMBDA="${ARM_IK_LAMBDA:-0.02}" \
  `# hand-obs 정합: 관측을 hand-only 판(61/53/206)과 동일하게 -- 팔 DOF 제외,` \
  ...
  `# Allegro V4 스펙: 관절당 최대 0.70 Nm (2.0 은 dg5fs 스톨 토크였음)` \
  CAP_DOF_EFFORT="${DOF_EFFORT:-0.7}" \
  `# 손목도 변화량: 내부 목표를 스텝당 pos 5mm / rot 0.05rad 까지 옮기고 PD 로` \
  `# 추종. 액션 0 = 손목 유지. 랜덤 초기 정책이 파지를 못 깨게 하는 짝.` \
  CAP_WRIST_DELTA="${WRIST_DELTA:-0}" \
  ...
  `# 해제 래치가 원시 조인트각 기준(screw_coupling)이라 공차 불필요 -- 180 복원.` \
  `# (Al6의 179 공차는 누적각 3도 지연에 대한 붕대였음)` \
  ...
  `# 낙하 종료 일시불 페널티 (자살 탈출 차단 -- Arm5 ep길이 붕괴 실측 처방)` \
  CAP_DROP_FAIL_PENALTY="${DROP_FAIL_PENALTY:-300}" \
  ...
  `# allegro 고무 팁 반경 (URDF tip mesh 오프셋 12mm 기준)` \
  ...
  `# allegro 3지 = 3짝뿐 (dg5fs 6짝 4.0e-3). 초기 추정 -- 파지 자세 나오면 실측 재보정` \
  CAP_CLOSURE_AREA_REF="${CLOSURE_AREA_REF:-2.0e-3}" \
