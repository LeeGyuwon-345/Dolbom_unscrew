#!/usr/bin/env bash
# tools2 본 학습 watchdog. tools 판(watch_main180.sh)의 이식 --
# 로그 패턴, 실험명, 재시작 스크립트만 tools2 것으로 바뀌었다.
#
...
TOOLS=/home/leegyuwon/Documents/task1/tools2_allegro_allegro
...
trainer_pid() {
  for p in $(pgrep -f "main/rl/train.py" 2>/dev/null); do
    ...
    case "$c" in
      /home/leegyuwon/*python*experiment=CapUnscrewAllegro_${TAG}_*) echo "$p"; return;;
    esac
  done
}

latest_log() { ls -t "${LOGS}/cap_unscrew_allegro_${TAG}_"*.log 2>/dev/null | head -1; }
latest_ckpt() { ls -t "${RUNS}/CapUnscrewAllegro_${TAG}_"*/nn/*.pth 2>/dev/null | head -1; }
...
epochs_done() {
  local best=0 n
  for l in "${LOGS}/cap_unscrew_allegro_${TAG}_"*.log; do
    [[ -f "$l" ]] || continue
...
while :; do
  ...
  if [[ -n "$ck" ]]; then
    RESAMPLE_WRIST=1 MAX_ITER="$remain" NUM_ENVS=512 DIAG_EVERY=200 TAG="$TAG" CKPT="$ck" \
      setsid nohup ./train_cap_unscrew_allegro.sh > /dev/null 2>&1 < /dev/null &
  else
