...
trainer_pid() {
  for p in $(pgrep -f "main/rl/train.py" 2>/dev/null); do
    ...
    case "$c" in
      /home/leegyuwon/*python*experiment=CapUnscrewArm_${TAG}_*) echo "$p"; return;;
    esac
  done
}

latest_log() { ls -t "${LOGS}/cap_unscrew_arm_${TAG}_"*.log 2>/dev/null | head -1; }
latest_ckpt() { ls -t "${RUNS}/CapUnscrewArm_${TAG}_"*/nn/*.pth 2>/dev/null | head -1; }
...
epochs_done() {
  local best=0 n
  for l in "${LOGS}/cap_unscrew_arm_${TAG}_"*.log; do
    [[ -f "$l" ]] || continue
...
while :; do
  ...
  if [[ -n "$ck" ]]; then
    RESAMPLE_WRIST=1 MAX_ITER="$remain" NUM_ENVS=512 DIAG_EVERY=200 TAG="$TAG" CKPT="$ck" \
      setsid nohup ./train_cap_unscrew_arm.sh > /dev/null 2>&1 < /dev/null &
  else
