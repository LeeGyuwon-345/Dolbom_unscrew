"""tools2 판 -- CAP_GRASP_TOOLS_DIR 가 이 폴더를 가리키면 tools 의 동명 모듈 대신
이 파일이 로드된다 (환경이 tools 를 sys.path 뒤에 fallback 으로 두므로, 여기
없는 공용 모듈은 그대로 tools 에서 온다).

시작점은 tools 판의 사본이다. 성공 조건과 보상 구성은 여기서 다시 짠다;
아래 원본 주석은 참고용으로 남겨둔다.
...
class CapUnscrewCurriculumConfig:
    ...
    # few degrees.
    # tools2: 방향(palm cos)을 계단이 아니라 가중치로. cos 0.8 게이트는
    # from-scratch 부트스트랩에 세 번 실패했고, press/pinch/near 는 같은
    # 처방(마스크->계수)으로 고쳐져 작동했다. 성공이 "해제" 로 바뀌면서
    # 게이트의 원래 근거(해제 후 캡 운반 마찰)도 절반은 사라졌다.
    # 힘·밴드는 계단 유지 -- 닿았냐는 사실이고, 방향만 "얼마나 잘" 이다.
    palm_grade: bool = True          # tools2 기본 = 현재 학습 방식 (Main4)
    palm_grade_pow: float = 2.0
    # closure 를 대향각 대신 "접촉점 부채꼴 삼각형 넓이" 로 계산 (Main10 실험).
    # 엄지를 꼭짓점으로 (검지,중지)(중지,약지)(약지,새끼) 세 삼각형의 3D 넓이에
    # 세 꼭짓점의 접촉 가중치를 곱해 합산 -- 안 닿은 꼭짓점의 삼각형은 0 이라
    # "공중에 펼치기" 로는 벌 수 없다. grasp_init/optimize_pose 의
    # contact_fan_area 에서 검증된 형태의 RL 이식.
    #
    # 위치 처방(엄지는 중지 반대편)을 결과로 바꾼다: 넓이가 크려면 접촉이
    # 펼쳐져야 하고, 어느 손가락이 어디로 갈지는 기하가 정한다. 한쪽 몰림은
    # 자동 기각 (80도 안 3접촉 = 기준의 ~8%).
    closure_area: bool = False
    # 수렴 4지 파지의 가중 넓이 실측(~1500-2000mm^2)에 맞춘 정규화. 정오각형
    # (5944)을 쓰면 수렴해도 0.3 대라 수입이 반토막 난다.
    closure_area_ref: float = 4.0e-3        # m^2  (6짝 합산 기준)
    # allegro(4지): env 의 5번째 손가락 슬롯이 약지 중복이라는 표시.
    # 접촉 가중에서 슬롯 4 를 죽여 이중 계산을 막는다. 비엄지 3지의
    # 모든 짝은 (1,2),(1,3),(2,3) 3개 -- 슬롯 4 가중치가 0 이면 전 코드가
    # 자동으로 그 짝만 남기므로 삼각형 코드는 그대로다.
    # env 가 이 필드를 kwargs 로 넘기지 않으므로 (구 tools 호환 유지)
    # import 시점의 환경변수를 기본값으로 쓴다 -- 런처가 세팅한다.
    dup_last_finger: bool = bool(int(os.environ.get("CAP_DUP_LAST_FINGER", "0")))
    ...
    # 성공조건에 "해제 후 실제 유지"를 요구하는 시간(초). 0 이면 기존처럼
    # 해제 순간 성공. 켜면 hold_timer 가 높이(lifted) 대신 "해제됨 AND 안정
    # 파지"를 세고, 그 시간이 이 값을 넘어야 성공이다 -- free6 물리에서
    # 해제 즉시 떨어뜨리는 정책이 성공으로 집계되는 것을 막는다.
    released_hold_success_time: float = 0.0
...
def cap_unscrew_curriculum_reward(
    ...
    touch = raw_contact & on_band                       # 물리 사실 (계단)
    if cfg.dup_last_finger:
        # allegro: 4지 손이라 env 가 5번째 슬롯에 약지를 중복해 채운다.
        # 중복 슬롯이 접촉 수·삼각형·quality 를 이중 계산하지 않게 죽인다.
        # (near/pinch 의 거리 항은 중복이 평균에 살짝 섞여도 무해)
        touch = touch.clone()
        touch[:, 4] = False
    tip_contact = touch & palm_ok                        # 진단용 "인정 접촉" (0.8 계단)
    if cfg.palm_grade:
        # 방향은 가중: cos^pow. 0.79 가 0 이 되고 0.81 이 만점이 되는 절벽을
        # 없애고, 낮은 cos 접촉(검지 0.64 등)도 부분 기여하게 한다.
        w_dir = press_dir ** cfg.palm_grade_pow
        contact_w = touch.to(dtype=cap_pos_w.dtype) * w_dir      # (N,5) 실수
    else:
        contact_w = tip_contact.to(dtype=cap_pos_w.dtype)
    contact_count = contact_w.sum(dim=-1)                # 실수
    thumb_contact = tip_contact[:, 0]                    # 진단용 bool 유지
    thumb_w = contact_w[:, 0]
    # 대향 오차의 접촉 마스크는 방향과 무관하게 "캡 위에 있는 손가락" 기준.
    closure_err = thumb_opposition_err(tip_pos_w, cap_pos_w, cap_quat_xyzw, touch, cfg)
    ...
    grasp_ok = thumb_contact & (tip_contact.to(dtype=torch.int32).sum(dim=-1) >= cfg.min_contact_tips)
    if cfg.palm_grade:
    ...
    if cfg.closure_area:
        # 접촉 부채꼴 넓이. 손끝 위치를 캡 프레임 xy 로 옮겨 엄지 꼭짓점
        # 삼각형 3개의 넓이 x 꼭짓점 접촉 가중치 곱을 합산한다.
        rel_t = cap_local_probes(tip_pos_w, cap_pos_w, cap_quat_xyzw)[:, :, -1, :]  # (N,5,3) tip
        # 비엄지 모든 짝 6개. 인접 3짝(부채꼴)은 중간 사슬 손가락이 빠지면
        # 양옆 삼각형까지 같이 죽는다 -- 검지가 상시 미접촉이라 실질 2개,
        # 중지까지 빠지면 1개만 남아 잘 벌린 파지도 과소평가됐다. 모든 짝이면
        # 어느 하나가 빠져도 나머지 조합이 넓이를 담는다. 겹침 과대계산은
        # ref 를 그만큼 키워(2e-3 -> 4e-3) 상쇄한다.
        area = None
        for a, b in ((1, 2), (1, 3), (1, 4), (2, 3), (2, 4), (3, 4)):
            u = rel_t[:, a] - rel_t[:, 0]
            v = rel_t[:, b] - rel_t[:, 0]
            tri = 0.5 * torch.linalg.cross(u, v, dim=-1).norm(dim=-1)
            wtri = contact_w[:, 0] * contact_w[:, a] * contact_w[:, b]
            term = tri * wtri
            area = term if area is None else area + term
        closure_term = (area / max(cfg.closure_area_ref, 1e-9)).clamp(0.0, 1.0)
    else:
    ...
    grasp_quality = closure_term * contact_frac * (
        thumb_w if cfg.palm_grade else thumb_contact.to(dtype=cap_pos_w.dtype))
    ...
    # 해제 래치. 없으면(구 경로) frac >= 1 순간 판정으로 대체.
    rel_f = released_latch if released_latch is not None else (
        (frac >= 1.0).to(dtype=cap_pos_w.dtype))
    # 해제되면 unscrew 지급을 최대치로 고정한다. 없을 때의 실측: 해제 후
    # 자유로워진 캡이 되감기면 frac 이 떨어져 "해제 = 수입 불안정" 이 되고,
    # 정책은 나사산이 각도를 보장하는 174도 주차를 배웠다 (ep300 17.2% ->
    # ep600 0.6%, 보상은 계속 상승). 래치면 해제 후 수입이 되감김과 무관하게
    # 주차(9.7q)보다 항상 크다.
    pay_eff = torch.maximum(pay_frac, rel_f) if not cfg.turn_progress else pay_frac
    reward_unscrew = pay_eff * grasp_quality * grasp_hold * grasp_ok_f
    ...
    # tools2 의 hold: "해제 후에도 잡고 있기". 원판(16mm 들고 0.3초)은 성공
    # 조건 변경으로 죽은 항이라 자리를 재사용한다. released 는 래치라 캡이
    # 손안에서 되감겨도 유지 수입이 흔들리지 않는다 -- unscrew 래치와 함께,
    # 해제+유지(14q)가 주차(9.7q)를 모든 경우에 이기게 만드는 두 축이다.
    reward_hold_released = rel_f * grasp_quality * grasp_hold
    ...
    # that from counting.
    if cfg.released_hold_success_time > 0.0:
        # 성공조건용 유지: 해제된 채 안정 파지를 이어간 시간을 센다. 높이
        # (lifted) 기준은 free6 에서 의미가 없다 -- 캡이 자유라 손에 없어도
        # 높이가 남을 수 있고, 떨어뜨리면 stable 이 꺼져 타이머가 감쇠한다.
        hold_frame = (rel_f > 0.5) & stable
    else:
        hold_frame = lifted
    updated_hold = torch.where(
        hold_frame, hold_timer + dt, (hold_timer - cfg.hold_decay_rate * dt).clamp_min(0.0)
    )
    # tools2 성공 조건: 캡 해제(180도, screw_coupling 구속 해제와 같은 순간).
    # released_hold_success_time > 0 이면 거기에 "해제 후 그 시간만큼 실제
    # 유지"가 추가된다. 성공 래치는 env(NO_RESET_ON_SUCCESS)가 담당하므로
    # 조건 충족 후 떨어뜨려도 성공 기록은 유지된다.
    # (원판은 16mm 들고 0.3초 유지였다: updated_hold >= cfg.hold_time)
    if cfg.released_hold_success_time > 0.0:
        # 해제 래치 기준으로만 판정한다. unscrewed(frac>=1, 목표각 180 기준)와
        # 엮으면 ENGAGE_DEG(해제각)를 179 로 내렸을 때 "해제는 되는데 성공은
        # 영영 불가" 모순이 생긴다 (Al6 ep500 success 정확히 0 실측).
        # hold_frame 이 이미 (해제 AND 안정 파지)에서만 타이머를 올리므로
        # 타이머 조건 하나가 곧 "해제 후 0.5초 실제 유지"다.
        success = updated_hold >= cfg.released_hold_success_time
    else:
        success = unscrewed
    ...
    # encoding.
    # tools2: hold / lift / release 는 목적에서 제거. 성공이 "캡 해제" 그
    # 순간이므로 그 뒤를 채점하는 hold / lift 는 죽은 항이고, release 는 성공
    # 판정과 같은 사건이라 보상으로 또 줄 이유가 없다. 계산은 남겨 진단에서
    # 계속 읽는다 (reward_dict 에 hold/lift/release 가 0 아닌 값으로 보이면
    # 무언가 잘못 켜진 것).
    objective = (
        cfg.unscrew_weight * reward_unscrew
        + cfg.hold_weight * reward_hold_released
        ...
        # tools2: press 제거. 초기자세가 이미 엄지+반대편 핀치 구조를 만족한
        # 채 시작하므로 "쥐기 시작해라" 는 당근이 필요 없다. 힘 부족은 제약
        # 쪽 grip 결손이 계속 벌점으로 잡는다. 계산은 진단용으로 남긴다.
    )
    ...
    terms = {
        ...
        "reward_hold_released": reward_hold_released,
