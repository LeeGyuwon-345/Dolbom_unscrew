            ...
            data.update(
                {
                    ...
                    # pkl 없을 때 fallback: 리타겟 keypoint를 0으로 (B방식은 pkl 존재 전제)
                    "opt_joints_pos": torch.zeros(
                        [data["wrist_pos"].shape[0], len(self.dexhand.body_names), 3], device=self.device
                    ),
                }
            )
        else:
            ...
            data.update(
                {
                    ...
                    "opt_joints_pos": torch.tensor(opt_params["opt_joints_pos"], device=self.device),  # 리타겟 keypoint (B방식 타겟)
                }
            )
