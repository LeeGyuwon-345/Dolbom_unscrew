...
class DexHandImitatorRHEnv(VecTask):
    ...
    def __init__(
        ...
        *,
        ...
        # B방식: MANO 대신 리타겟 결과(opt_*)를 imitation 타겟으로. 환경변수 IMITATE_RETARGET=1
    ...
    def _create_envs(self):
        ...
        # B방식: imitation 타겟을 MANO -> 리타겟 결과(opt_*)로 교체.
        # opt_joints_pos는 (E,T,26,3) body_names순 -> base 제외한 [1:]가 packed mano_joints(25 body)와 1:1 대응.
        if self.imitate_retarget:
            ...
            print("[imitate_retarget] imitation 타겟 = 리타겟 결과(opt_*) (B방식)")
    ...
    def _reset_default(self, env_ids):
        ...
        # B방식: 초기 DOF를 리타겟 관절각(opt_dof_pos)의 시작 프레임에서 (기본자세 대신) → 좋은 자세에서 출발
        if self.imitate_retarget:
            base_dof_pos = self.demo_data["opt_dof_pos"][env_ids, seq_idx]
        else:
            base_dof_pos = self.dexhand_default_dof_pos[None].repeat(len(env_ids), 1)

        dof_pos = torch.clamp(
            base_dof_pos + noise_dof_pos,
...
def compute_imitation_reward(
    ...
    failed_execute = (
        (
            # dg5fs 형상한계 반영해 손끝 종료임계 완화 (리타겟 per-frame 오차 ~90th 기준).
            # 원본(inspire): thumb0.04 index0.045 middle0.05 pinky/ring0.06.
            (diff_thumb_tip_pos_dist > 0.09 / 0.7 * scale_factor)
            | (diff_index_tip_pos_dist > 0.09 / 0.7 * scale_factor)
            | (diff_middle_tip_pos_dist > 0.09 / 0.7 * scale_factor)
            | (diff_pinky_tip_pos_dist > 0.07 / 0.7 * scale_factor)
            | (diff_ring_tip_pos_dist > 0.08 / 0.7 * scale_factor)
